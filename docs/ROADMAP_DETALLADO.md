...
### **Fase 2: Expansión y Refinamiento del "Motor" y QA (Q1 2026)**

**Objetivo Principal:** Aumentar la base de conocimiento del dataset mientras se mejoran las herramientas de procesamiento y los mecanismos de control de calidad.
...
*   **Hitos y Tareas Clave:**
    ...
    3.  **Implementación de QA Avanzado:**
        *   **Validación Semántica:** Mejorar `validate_chunks.py` para incluir chequeos semánticos (ej. rangos numéricos lógicos, consistencia de términos).
        *   **Detección de Duplicados:** Implementar la lógica en `scripts/merge_redundant.py` para detectar y reportar chunks con alta similitud.
    4.  **Implementación de Pruebas RAG:**
        *   Desarrollar `scripts/test_retrieval.py` para cargar los chunks en LanceDB y ejecutar consultas de prueba.
        *   Integrar este script en el workflow `test-rag.yml` para validar que los nuevos chunks son recuperables y no degradan el rendimiento.
    5.  **Documentación de Hallazgos:**
        *   Crear `docs/LLM_GUIDANCE.md` para documentar qué LLMs y prompts ofrecen los mejores resultados para la generación de chunks.
        *   Crear `docs/CONFLICT_RESOLUTION.md` para explicar cómo se manejan las contradicciones entre fuentes.

*   **Entregable:** Un dataset con ~150 chunks, herramientas de QA más robustas y un pipeline de CI/CD que incluye pruebas de recuperación RAG.
*   **Criterio de Éxito:** El dataset cubre >70% de la taxonomía. Los scripts de QA pueden detectar y reportar errores semánticos y duplicados.

---

### **Fase 3: Release v1.0 y Apertura a la Comunidad (Finales Q1 2026)**
...
*   **Hitos y Tareas Clave:**
    1.  **Generación de Embeddings:** Ejecutar `scripts/compute_embeddings.py` para generar los vectores para todos los chunks del dataset.
    2.  **Configuración del Workflow de Release:** Validar que `publish-release.yml` empaqueta correctamente `chunks_enriched_v1.0.jsonl` y `vectors_300d_v1.0.parquet`.
    3.  **Creación del Tag `v1.0`:** Crear y subir un tag `v1.0` para disparar la creación de la primera release oficial en GitHub.
    4.  **Refinamiento de Documentación para Contribuidores:** Realizar una revisión final de `CONTRIBUTING.md` y las plantillas de `ISSUE_TEMPLATE` para asegurar una experiencia de contribución fluida.
    5.  **Creación de Ejemplos de Uso:** Añadir ejemplos prácticos en `docs/examples/`, como `load_with_lancedb.py` y `query_example.py`.

*   **Entregable:** Una release `v1.0` oficial en GitHub con el dataset, embeddings y ejemplos de código listos para descargar y usar.
*   **Criterio de Éxito:** Un desarrollador externo puede descargar la release, seguir los ejemplos y tener un sistema RAG básico funcionando en menos de 30 minutos.
